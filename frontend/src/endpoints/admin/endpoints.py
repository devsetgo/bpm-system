...
@login_required.require_login
async def login_attempts_page(request):

    since_date = datetime.utcnow() - timedelta(days=30)
    fail_query = user_login_failures.select().where(
        user_login_failures.c.date_created >= since_date
    )
    data = await fetch_all_db(query=fail_query)

    template = f"{page_url}/login_failures.html"
    context = {
        "request": request,
        "data": data,
        "active": "login-failures",
        "section": section,
        "page_name": "Login Failures",
    }
